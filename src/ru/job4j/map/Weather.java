...
public class Weather {
    public static List<Info> editData(List<Info> list) {
        ...
        list.forEach(user -> {
            map.computeIfPresent(user.getCity(), (city, rainfall) -> rainfall + user.getRainfall());
            map.putIfAbsent(user.getCity(), user.getRainfall());
        });
        map.forEach((key, val) -> rsl.add(new Info(key, val)));
        return rsl;
    }
